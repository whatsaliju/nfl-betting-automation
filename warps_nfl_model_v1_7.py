"""
WARPS-NFL v1.7 — Champion Weight Search + 3-Model Consensus Screen

v1.6 finding: pyth_edge is the dominant signal; pass_pyth composite
(40% pass EPA + 60% pyth edge) is the best composite on validation.

v1.7 strategy:
  Phase 1 : Fine 3-component grid (pyth_edge, pass_epa, point_diff) — 231 configs
  Phase 2 : Biased Dirichlet search (high-pyth region, n=300, all 7 components)
  Phase 3 : Full hyper grid for top-5 weight candidates
  Phase 4 : Validate on 2022-2025, select champion
  Phase 5 : Full backtest 2015-2025 + 2026 screen
  Phase 6 : Consensus screen — v1.5d ∩ v1.6 ∩ v1.7 overlapping Strong signals

Install:  pip install pandas numpy nfl_data_py
Run:      python warps_nfl_model_v1_7.py [--overrides warps_2026_overrides.csv]
"""
...
def run(overrides_csv="warps_2026_overrides.csv"):
    print("=" * 70)
    print("WARPS-NFL v1.7 — Champion Weight Search + Consensus Screen")
    print("=" * 70)
    ...
    g3.to_csv("warps_fine_3comp_grid_v1_7.csv", index=False)
    best3 = g3.iloc[0]
    ...
    bd.to_csv("warps_biased_dirichlet_v1_7.csv", index=False)
    best_bd = bd.iloc[0]
    ...
    best_grid.to_csv("warps_parameter_grid_v1_7.csv", index=False)
    ...
    full_res.to_csv("warps_backtest_team_results_v1_7.csv", index=False)
    pd.DataFrame([val_m]).to_csv("warps_validation_metrics_v1_7.csv", index=False)
    ...
    by_year.to_csv("warps_backtest_by_year_v1_7.csv", index=False)

    cal = calibration_buckets(full_res)
    cal.to_csv("warps_calibration_buckets_v1_7.csv", index=False)
    ...
    scr.sort_values("edge", ascending=False).to_csv("warps_2026_screen_v1_7.csv", index=False)
    games.to_csv("warps_2026_game_probs_v1_7.csv", index=False)
    ...
    consensus.to_csv("warps_consensus_screen_v1_7.csv", index=False)
    ...
    print("\n" + "=" * 70)
    print("BY-YEAR: WARPS v1.7 vs Pythagorean vs Prior Wins")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("CONSENSUS SCREEN — v1.5d + v1.6 + v1.7")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"  v1.5d  reg=0.65, logit=6.5  val_MAE=2.534  (composite default)")
    print(f"  v1.6   reg=0.75, logit=5.5  val_MAE=2.512  (solo_pyth_edge champion)")
    print(f"  v1.7   reg={reg_f},  logit={logit_s}  val_MAE={val_m['warps_mae']:.3f}  ({best_name})")
    improvement = 2.534 - val_m["warps_mae"]
    print(f"\n  v1.7 improvement over v1.5d: {improvement:+.3f} wins/team on validation")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="WARPS-NFL v1.7")
