#!/usr/bin/env python3
"""Refresh the WARPS market overlay end to end.
...
def get_season(season_arg: int | None) -> int:
    ...
    try:
        ...
        pass
    current_week_path = ROOT / "data" / "current_week.json"
    if current_week_path.exists():
        data = json.loads(current_week_path.read_text())
        if "season" in data:
            return int(data["season"])
    raise SystemExit("Could not determine current season; pass --season explicitly")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Refresh WARPS market overlay")
    parser.add_argument("--season", type=int, default=None, help="NFL season year (default: auto-detect from get_current_season())")
