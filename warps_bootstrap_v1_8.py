"""
WARPS-NFL v1.8 — Bootstrap CI + Diebold-Mariano Statistical Validation

Runs on top of warps_backtest_team_results_v1_8.csv (already generated).
No re-running the model — pure statistical analysis of forecast errors.

Install:  pandas numpy (already installed)
Run:      python warps_bootstrap_v1_8.py

Output:
  warps_bootstrap_results_v1_8.csv   — per-bootstrap MAE samples
  warps_statistical_summary_v1_8.txt — publication-ready summary table
"""
...
def run():
    ...
    try:
        ...
        sys.exit(f"ERROR: {results_path} not found. Run warps_nfl_model_v1_8.py first.")
    ...
    print("=" * 70)
    print("WARPS-NFL v1.8 — Statistical Validation")
    print("=" * 70)
    ...
    print("\n" + "─" * 70)
    print("POINT ESTIMATES — Full Backtest (2000-2025)")
    print("─" * 70)
    ...
    print(f"  {'─'*25}  {'─'*6}  {'─'*20}")
    print(f"  {'WARPS v1.8':<25}  {warps_mae_full:>6.3f}  [{ci_w[0]:.3f}, {ci_w[1]:.3f}]")
    ...
    print(f"  {'Model':<25}  {'MAE':>6}  {'95% CI':>20}")
    print(f"  {'─'*25}  {'─'*6}  {'─'*20}")
    print(f"  {'WARPS v1.8':<25}  {warps_mae_val:>6.3f}  [{ci_wv[0]:.3f}, {ci_wv[1]:.3f}]")
    ...
    out.to_csv("warps_bootstrap_results_v1_8.csv", index=False)

    # ── Save text summary ────────────────────────────────────────────────────
    summary_lines = [
        "WARPS-NFL v1.8 Statistical Validation Summary",
        "=" * 55,
        "",
    ...
    summary_text = "\n".join(summary_lines)
    with open("warps_statistical_summary_v1_8.txt", "w") as f:
        f.write(summary_text)

    print("\n" + "─" * 70)
    print("Outputs written:")
    print("  warps_bootstrap_results_v1_8.csv")
    print("  warps_statistical_summary_v1_8.txt")
